...
export async function executeRestApi(
  ...
  // Use Playwright's request context - check multiple possible locations
  const request: APIRequestContext = world.fixture?.request || world.request;
  if (!request) {
    console.log('Available world properties:', Object.keys(world));
    console.log('World.fixture:', world.fixture);
    throw new Error('Playwright APIRequestContext not found in world. Make sure the @api tag is present and the Before hook is executed.');
  }
  ...
  if (expectedStatusCode && expectedStatusCode !== '' && response.status() !== parseInt(expectedStatusCode)) {
